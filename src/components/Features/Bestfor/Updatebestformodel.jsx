...
const Updatebestformodel = ({closeBestforModel, opencreatebestformodel, opendeletebestformodel}) => {
    ...
    const { apiResponse, handlSubmitRequest, apiData } = useApiRequest(requesttype.url, requesttype.type, bestfor);
  ...
  const handleUpdateBestfor = () => {
        if(!bestfor.name || bestfor.name.length < 3) {
            ...
            setRequesttype({
                ...
                url: `/categories/${bestfor.id}`
            });
        }
  };


  const handleDeleteBestfor = () => {
       if(!bestfor.id) {
        setValidationErrors({
          id: "Please select a bestfor to delete"
        });
       } else {
        setRequesttype({
      ...
      >
        ...
        <section className="space-y-4">
          ...
          <div className="line-mb:flex space-y-2 line-mb:space-y-0 items-end justify-between gap-3">
            ...
            <Select
           options={apiData?.categories.map((bestfor) => bestfor.name)}
           idoptions={apiData?.categories.map((bestfor) => bestfor._id)}
          label="Select Bestfor"
          className=""
...
export default Updatebestformodel