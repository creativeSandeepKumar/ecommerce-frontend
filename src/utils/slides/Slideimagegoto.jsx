...
const Slideimagegoto = ({productimage, goToSlide, currentIndex, isleftsidebar="false"}) => {
  return (
    <div className={`absolute bottom-2 left-0 right-0 md:right-auto md:top-0 flex justify-center items-center max-w-lg mx-auto  py-1 rounded-md md:overflow-y-auto`}>
    <Carousel
        className="w-full">
      <CarouselContent className="w-full md:flex md:flex-wrap md:max-w-24 items-center justify-center h-full gap-2 ">
        {productimage.map((item, index) => (
          <CarouselItem key={index} className="max-w-24">
            <div className="">
              <Card className="">
                <CardContent className="aspect-square items-center justify-center">
                <img onClick={() => goToSlide(index)} src={item} alt={""} className={`min-h-[70px] h-full min-w-[60px] w-full rounded-sm ${
